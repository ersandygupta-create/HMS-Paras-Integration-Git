page 50106 "EDC Posted HIS GRN Capex List"
{

    ApplicationArea = All;
    Caption = 'Posted HIS GRN Capex List';
    PageType = List;
    Editable = false;
    CardPageId = 50013;
    DelayedInsert = false;
    InsertAllowed = false;
    DeleteAllowed = false;
    SourceTable = "EDC HIS Purchase Header";
    SourceTableView = sorting("Entry No.") where("Record Type" = Filter(GRN), "Document Type" = filter(Order), "Capex Type" = filter(Capex), "Create PO" = filter(true));
    UsageCategory = Lists;
